...
class Scheduler {
  ...
  run() {
    ...
    const webpackPlugin = this.serverless.pluginManager.getPlugins().find(p => {
      return !!p.webpackOutputPath;
    });
    if (webpackPlugin) {
      this.absoluteLocation = webpackPlugin.webpackConfig.output.path;
    }
  ...
  _requireFunction(fName) {
    ...
    let funcPath = path.join(
      this.serverless.config.servicePath,
      this.location || "", filename);
    if (this.absoluteLocation) {
      funcPath = path.join(this.absoluteLocation, filename);
    }
